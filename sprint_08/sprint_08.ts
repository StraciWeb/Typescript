...
const f_10 = (): void => {
  for(let i = 0; i<obj_10.data.length; i++) {
    if(obj_10.data[i].length !== 6 ) {
        obj_10.data.splice(i, 1);
    }
  }
}
...
const f_12 = (): { title: string, step: number, defense: number, stamina: number } => {
    return {"title": "Древний огр", step: 1, defense: 8, stamina: 14};
}
