...
class Paragraph_03 {
    ...
    render() : HTMLParagraphElement {
        ...
        if(this.bold) {
            p1.append(b1)
            b1.textContent = this.text;
        }else  p1.textContent = this.text;
        return p1;
    }
}
...
class Paragraph_05 {
    ...
    render() : HTMLParagraphElement {
        ...
        if(this.bold) {
            p1.append(b1)
            b1.textContent = this.text.charAt(0).toUpperCase() + this.text.slice(1);
        }else  p1.textContent = this.text.charAt(0).toUpperCase() + this.text.slice(1);
        return p1;
    }
...
class List_09 {
    ...
    render() : HTMLUListElement {
        const list = document.createElement("ul");
        this.cssClass.forEach(item => list.classList.add(item));
        this.data.forEach((element) => {
            const item = document.createElement("li");
            item.textContent = element;
            list.append(item);
...
// тут пишем класс
class Cart {
    total (array : {articul: string, price: number, amount: number}[]) : number {
        let total = 0;
...
class CellOperator {
    ...
    operator() : string {
        if(this.code === '050' || this.code === '066' || this.code === '095' || this.code === '0999') return "Vodafone";
        if(this.code === '063' || this.code === '073' || this.code === '093') return "Life";
        if(this.code === '067' || this.code === '068' || this.code === '096' || this.code === '98') return "Kyivstar";
        else return "";
    }
}
...
class CellPhone {
    ...
    phone: string;
    constructor (text: string) {
        this.code = text.slice(3, 6);
        this.phone = text.slice(6);
    }
